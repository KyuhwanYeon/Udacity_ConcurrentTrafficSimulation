...
template <typename T> T MessageQueue<T>::receive() {
  ...
  // from the queue usingmove semantics. The received object should then be
...
TrafficLight::TrafficLight() {
     ...
     queueTrafLight = std::make_shared<MessageQueue<TrafficLightPhase> >();
     }

void TrafficLight::waitForGreen() {
 // std::shared_ptr<MessageQueue<TrafficLightPhase>> queue(new MessageQueue<TrafficLightPhase>);
  std::cout<<"CurrentTraffic   "<<std::endl;    
  while (true) {
   // std::shared_ptr<MessageQueue<TrafficLightPhase>> queue(new MessageQueue<TrafficLightPhase>);

    auto received = queueTrafLight->receive();

    if (received == TrafficLightPhase::green) {  // when traffic light = green -> break;
...
void TrafficLight::cycleThroughPhases() {
  ...
  while (true) {
    ...
    int rantime = 4 + (std::rand() % (6 - 4 + 1)); // randomnumber between 4~6

    timeDuration = std::chrono::duration_cast<std::chrono::seconds>(
                       std::chrono::system_clock::now() - previousTime)
                       .count();
    if (timeDuration > rantime) {
      if (_currentPhase == TrafficLightPhase::red) {
     //   std::cout << "Traffic light turn color to green  /   "<< "thread id = " << std::this_thread::get_id() << std::endl;
        _currentPhase = TrafficLightPhase::green;
      } else {
     //   std::cout << "Traffic light turn color to red  /   "<< "thread id = " << std::this_thread::get_id() << std::endl;

        _currentPhase = TrafficLightPhase::red;
      }
      auto msg = getCurrentPhase();

      queueTrafLight->send(std::move(msg));
